...
register_center(){
  ...
  local r=direct
  role_has relay && r=relay
  role_has center && role_has relay && r=all
  /usr/local/lib/vvv/register_sync.sh "$r" "$code"
}
show_sync(){
  if [[ -f /etc/vvv/client.json ]]; then
    jq '{base_url,host_id,role,registered_at,last_sync,last_result}' /etc/vvv/client.json
  else echo "尚未注册订阅中心。"; fi
}
update_vvv(){
  echo "正在重新下载最新 VVV 安装程序……"
  curl -fsSL --retry 5 "https://raw.githubusercontent.com/weizhiok/vvv/main/install.sh?v=$(date +%s)" | bash
}
landing_menu(){
  [[ -x /usr/local/sbin/vvv-landing-original ]] && exec /usr/local/sbin/vvv-landing-original
  echo "中转副机已安装，但原管理命令不存在。"; exit 1
}
...
while true; do
  ...
  if [[ -f /etc/vvv/client.json ]]; then echo "$n. 立即同步订阅"; act[$n]=sync; ((n++)); echo "$n. 查看订阅同步状态"; act[$n]=sync_status; ((n++));
  else echo "$n. 注册到订阅中心"; act[$n]=register; ((n++)); fi
  echo "$n. 检查并升级 VVV"; act[$n]=update; ((n++)); echo "0. 退出"
  read -r -p "请输入编号：" x
  [[ "$x" == 0 ]] && exit 0
  case "${act[$x]:-}" in
    ...
    sync_status) show_sync; systemctl --no-pager --full status vvv-sync.timer vvv-sync.path 2>/dev/null || true; pause;;
