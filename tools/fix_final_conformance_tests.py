...
legacy_assertion = "    for token in ('xray_dynamic_parts()', 'xray_hot_apply()'):\n        require(token not in prepare, f'仍保留旧 Xray API 热更新实现：{token}')\n"
if legacy_assertion in text:
    text = text.replace(legacy_assertion, '', 1)
elif '仍保留旧 Xray API 热更新实现' in text:
    raise SystemExit('unexpected legacy API assertion form')
conformance.write_text(text, encoding='utf-8')

runtime = Path('tests/final_runtime_validation.sh')
text = runtime.read_text(encoding='utf-8')
anchor = '''bash -n "$WORK/host.sh"
sh -n "$WORK/landing.sh"
bash -n "$WORK/center.sh"
'''
replacement = '''bash -n "$WORK/host.sh"
sh -n "$WORK/landing.sh"
bash -n "$WORK/center.sh"
...
if anchor in text:
    ...
    raise SystemExit('rendered host validation anchor not found')
