...
set -Eeuo pipefail
umask 077
LOG=/tmp/vvv-conformance-contract-patch.log
...
if [[ $rc -ne 0 ]]; then
  git reset --hard HEAD
  mkdir -p validation
  cp "$LOG" validation/conformance-contract-patch.log
  git add validation/conformance-contract-patch.log
  git commit -m 'Record conformance contract patch failure'
  git push origin HEAD
fi
exit "$rc"
